...
void SnakeRun(Uint8 Dir)
{
			 ...
			 (SF->Pos.Y	  == S0->Pos.Y))
		{
				...
				if(S0->Pos.Sts == 0)
				{
						...
						else if(S0->Pos.Y == (RMAXW-1) && DeltY > 0)
						{
								S0->Pos.Sts = 1;
								S0N->Pos.Y = 0;		
						}						
								 ...
								 if(S0->Pos.X == 0 		 		 && DeltX < 0) S0N->Pos.X = RMAXL - 1;
						else if(S0->Pos.X == (RMAXL-1) && DeltX > 0) S0N->Pos.X = 0;							
				...
				else
				{
						...
						else if(S0->Pos.Y == (LMAXW-1) && DeltY > 0)
						{
								S0->Pos.Sts = 0;
								S0N->Pos.Y = 0;	
						}						
...
//								 if(S0->Pos.Y == 0 		 && DeltY < 0) S0N->Pos.Y = RMAXW - 1;
//						else if(S0->Pos.Y == RMAXW && DeltY > 0) S0N->Pos.Y = 0;							
//						else																		 S0N->Pos.Y = S0->Pos.Y + DeltY;
						...
						{					
								...
								//=== 按比例进行X坐标变换
								S0N->Pos.X = (Uint16)S0N->Pos.X * (LMAXL - 0) / (RMAXL - 0);
								S0N->Pos.Y = LMAXW - 1;
						}
						else if(S0->Pos.Y == (RMAXW-1) && DeltY > 0)
						{
				...
				{
								 if(S0->Pos.X == 0 		 		 && DeltX < 0) S0N->Pos.X = LMAXL - 1;
						else if(S0->Pos.X == (LMAXL-1) && DeltX > 0) S0N->Pos.X = 0;							
						else																		 		 S0N->Pos.X = S0->Pos.X + DeltX;
						...
						if(S0->Pos.Y == 0 && DeltY < 0) 
						{
								...
								//=== 按比例进行X坐标变换
								S0N->Pos.X = (Uint16)S0N->Pos.X * (RMAXL - 0) / (LMAXL - 0);
								S0N->Pos.Y = RMAXW - 1;
						}
						else if(S0->Pos.Y == (LMAXW-1) && DeltY > 0)
						{
...
//				S0N->Next = NULL;
		...
		//=== end：记录本次运动方向
		LED.Flag.bit.SnakeRunDirOld = LED.Flag.bit.SnakeRunDir;
}
